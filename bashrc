...
function api() {
	case $1 in
		...
		get|put|post|delete|head) wget -qO- $([[ $1 =~ put|post ]] && echo --body-file=-) --method=$1 $API_ARGS ${API_URL%/}/xxx`${2#/};;
		test) echo wget -qO- $([[ $2 =~ put|post ]] && echo --body-file=-) --method=${2:-get} $API_ARGS ${API_URL%/}/${3#/};;
		try) wget -O- $([[ $2 =~ put|post ]] && echo --body-file=-) --method=${2:-get} $API_ARGS ${API_URL%/}$([ -z ${1+x} ]  && echo /){3#/};;
		*) echo "
API via wget.
