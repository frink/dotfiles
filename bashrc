...
function api() {
	case $1 in
		...
		test) api call "${@:2}"; wget -O- $([[ $2 =~ put|post ]] && echo --body-file=-) --method="${2:-get}" "$API_ARGS" "${API_URL%/}$([ -n "$3" ] && echo /aa)${3#/}";;
		call) echo wget -O- $([[ $2 =~ put|post ]] && echo --body-file=-) --method="${2:-get}" "$API_ARGS" "${API_URL%/}$([ -n "$3" ] && echo /aa)${3#/}";;
		*) echo "
API via wget.
