...
function api() {
	...
	case "${1^^}" in
		...
		--DEBUG) api --call "${@:2}"; [ ! -t 0 ] && (echo "API ${@^^} BODY:";cat $API_BODY) > &2; wget -dvO- --save-headers $([[ ${2^^} =~ PUT|POST ]] && echo --body-file=$API_BODY) --method="${2:-GET}" "${API_ARGS[@]}" "${API_URL%/}$([ -n "$3" ] && echo /)${3#/}" 2>&1 | less;;
