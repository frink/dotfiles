...
function api() {
	case $1 in
		...
		test) echo wget -qO- $([[ $2 =~ put|post ]] && echo --body-file=-) --method=${2:-get} $API_ARGS ${API_URL%/}$([ -z "$3" ] && echo /aa)${3#/};;
		try) wget -O- $([[ $2 =~ put|post ]] && echo --body-file=-) --method=${2:-get} $API_ARGS ${API_URL%/}$([ -z "$3" ] && echo /aa)${3#/};;
		*) echo "
API via wget.
