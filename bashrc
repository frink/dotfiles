...
function api() {
	case ${1^^} in
		...
		--DRYRUN) echo wget -O- $([[ $2 =~ put|post ]] && echo --body-file=-) --method="${2:-GET}" $(for x in "${API_ARGS[@]}"; do echo "${x%=*}$([ -n "${x##*=}" ] && echo  ="'asdf${x##*=}'") "; done)"${API_URL%/}$([ -n "$3" ] && echo /)${3#/}";;
		--DEBUG) api --dryrun "${@:2}"; wget -O- $([[ $2 =~ put|post ]] && echo --body-file=-) --method="${2:-GET}" "${API_ARGS[@]}" "${API_URL%/}$([ -n "$3" ] && echo /)${3#/}";;
