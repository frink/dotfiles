...
function api() {
	...
	case "${1^^}" in
		...
		--CALL) echo wget -dvO- $([[ ${2^^} =~ PUT|POST ]] && echo --body-file=$API_BODY) --method="${2^^:-GET}" $(for x in "${API_ARGS[@]}"; do echo "${x%%=*}$([ -n "${x#*=}" ] && echo  ="'${x#*=}'") "; done)"'${API_URL%/}$([ -n "$3" ] && echo /)$(echo ${3#/} | cut -d? -f1)?$([ -n "$API_QUERY" ] && echo "$API_QUERY&")$(echo ${3#/} | cut -d? -f2)'";;
		--DEBUG) $(export API_ARGS=( --save-headers "$API_ARGS" ); api --call "$@");api --call "$@";;
